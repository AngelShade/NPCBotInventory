...
private:
    ...
    void TryStartBotCubeChannel(ObjectGuid botGuid)
    {
        ...
        // Emote visual announcement
        std::string message = bot->GetName() + " channels Shadow Grasp on Manticron Cube, banishing Magtheridon!";
        me->TextEmote(message);
...
struct npc_hellfire_channeler_magtheridon : public ScriptedAI
{
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
        ...
        // Combined automated Focus-Fire Coordination upgrade:
        CoordinateBotsFocus();
    }
    ...
    void JustSummoned(Creature* summon) override
    {
        ...
        // Combined automated Warlock Banish upgrade:
        if (summon->GetEntry() == 17454 || summon->GetEntry() == 17473)
        {
            CoordinateBanishForAbyssal(summon);
        }
    }

    void CoordinateBotsFocus()
    {
        _scheduler.Schedule(1s, [this](TaskContext context)
        {
            ...
            Creature* targetFocus = nullptr;
            std::list<Creature*> channelers;
            me->GetCreatureListWithEntryInGrid(channelers, NPC_HELLFIRE_CHANNELER, 100.0f);
            float lowestHP = 100.f;
            for (Creature* c : channelers)
            {
                if (c && c->IsAlive() && c->IsInCombat() && c->GetHealthPct() <= lowestHP)
                {
                    lowestHP = c->GetHealthPct();
                    targetFocus = c;
                }
            }
                        ...
                        if (Player* player = itr->GetSource())
                        {
                            if (player->GetBotMgr())
                            {
                                for (auto const& pair : *player->GetBotMgr()->GetBotMap())
                                {
                                    ...
                                    {
                                        if (bot->GetBotAI() && bot->GetBotAI()->HasRole(BOT_ROLE_DPS | BOT_ROLE_RANGED) && !bot->GetBotAI()->HasRole(BOT_ROLE_TANK))
                                        {
                                            bot->SetTarget(targetFocus->GetGUID());
                                            bot->GetMotionMaster()->MoveChase(targetFocus);
                                            bot->AI()->AttackStart(targetFocus);
                                        }
                                    }
                                }
                            }
    ...
    void CoordinateBanishForAbyssal(Creature* abyssal)
    {
        Map* map = me->GetMap();
        if (!map)
            return;
        ...
        for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
        {
            if (Player* player = itr->GetSource())
            {
                if (player->GetBotMgr())
                {
                    ...
                    {
                        Creature* bot = pair.second;
                        if (bot && bot->IsAlive() && bot->GetClass() == CLASS_WARLOCK)
                        {
                            bot->CastSpell(abyssal, 710, false); // Spell ID 710 is Banish
                            std::string message = bot->GetName() + " banishes the Burning Abyssal!";
                            me->TextEmote(message);
                            break;
                        }
                    }
